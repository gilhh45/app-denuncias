...
function nextLevelPts(level) {
  const map = { Iniciante: 100, Confiável: 300, Verificado: 600, Especialista: 600 };
  return map[level] ?? 100;
}

function prevLevelPts(level) {
  const map = { Iniciante: 0, Confiável: 100, Verificado: 300, Especialista: 600 };
  return map[level] ?? 0;
}
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={[styles.credCard, { backgroundColor: c.card }]}>
          <View style={styles.credHeader}>
            <Text style={[styles.sectionLabel, { color: c.textMuted }]}>STATUS DE CIDADÃO</Text>
            <Feather name="shield" size={18} color="#00694d" />
          </View>
          <Text style={[styles.credLevel, { color: c.text }]}>{level}</Text>
          <Text style={[styles.credPts, { color: c.textMuted }]}>{user.credPoints} Pontos Acumulados</Text>
          {/* Progress bar */}
          <View style={[styles.progressBg, { backgroundColor: c.divider }]}>
            <View style={[styles.progressFill, { width: `${progressPct}%` }]} />
          </View>
          <View style={styles.progressLabels}>
            <Text style={[styles.progressCurrent, { color: '#00694d' }]}>NÍVEL ATUAL</Text>
            <Text style={[styles.progressNext, { color: c.textMuted }]}>{nextLevelLabel(level)}</Text>
          </View>
        </View>
          ...
          {recent?.length === 0 ? (
            ...
            recent.map(r => (
              <ActivityItem
                ...
                time={formatDate(r.data ||createdAt || Date.now())}
  ...
  return (
    <View style={[styles.actItem, { backgroundColor: c.cardWhite }]}>
      <View style={[styles.actThumb, { backgroundColor: c.divider }]}>
        {img && <Image source={img} style={styles.actThumbImg} resizeMode="cover" />}
      </View>
      <View style={styles.actInfo}>
        <Text style={[styles.actTitle, { color: c.text }]} numberOfLines={1}>{title}</Text>
...
const styles = StyleSheet.create({
  ...
  ctaText: { gap: 4 },
  ctaTitle: { fontFamily: 'HankenGrotesk_600SemiBold', fontSize: 24, color: '#fff', lineHeight: 31 },
  ctaSubtitle: { fontFamily: 'HankenGrotesk_400Regular', fontSize: 16, color: 'rgba(255,255,255,0.9)' },
  credCard: {
    borderRadius: 8,
    padding: 24,
    gap: 0,
  ...
  credLevel: { fontFamily: 'HankenGrotesk_700Bold', fontSize: 32, lineHeight: 38 },
  credPts: { fontFamily: 'JetBrainsMono_500Medium', fontSize: 14, marginTop: 4, marginBottom: 24 },
  progressBg: { height: 12, borderRadius: 12, overflow: 'hidden', marginBottom: 8 },
  progressFill: { height: '100%', backgroundColor: '#00694d', borderRadius: 12 },
  progressLabels: { flexDirection: 'row', justifyContent: 'space-between' },
  progressCurrent: { fontFamily: 'JetBrainsMono_700Bold', fontSize: 11, letterSpacing: 1.2 },
  progressNext: { fontFamily: 'JetBrainsMono_700Bold', fontSize: 11, letterSpacing: 1.2 },
  section: { gap: 8 },
  actItem: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  actTime: { fontFamily: 'JetBrainsMono_700Bold', fontSize: 11, letterSpacing: 1.2 },
});
