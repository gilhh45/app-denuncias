...
/**
 * PASSO 1 de 3: pede o e-mail cadastrado e "envia" o código.
 *
 * TODO (backend): sem banco de dados ainda, então qualquer e-mail com
 * formato válido segue em frente. Quando existir um backend, troque o
 * corpo desta função pela chamada real (ex.: Firebase, Supabase, API
 * própria), que deve gerar e enviar um código por e-mail.
 */
...
export default function RecuperarSenhaScreen() {
  ...
  async function handleSend() {
    ...
    setLoading(true);
    try {
      await enviarCodigoDeVerificacao(emailLimpo);
      router.push({ pathname: '/verificar-codigo', params: { email: emailLimpo } });
    } catch (e) {
  ...
  return (
    ...
    >
      ...
      >
        <Text style={[styles.heading, { color: c.text }]}>ESQUECEU A SENHA?</Text>
        <Text style={[styles.subheading, { color: c.textMuted }]}>
          Informe o e-mail cadastrado. Vamos enviar um código de verificação para redefinir sua senha.
        </Text>

        <View style={styles.form}>
          <View style={styles.fieldGroup}>
            <Text style={[styles.fieldLabel, { color: c.text }]}>E-MAIL CADASTRADO</Text>
            <TextInput
          ...
          >
            {loading ? <ActivityIndicator color="#fff" /> : (
              <>
                <Text style={styles.submitLabel}>ENVIAR E-MAIL</Text>
